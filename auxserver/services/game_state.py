...
class GameState:
    ...
    def _try_attack_player(self, attacker_pid, target_pid):
        ...
        self._queue_ai_alert(target_pid, f"{attacker_pid} hit me for {dmg} damage.", f"{attacker_pid} struck me.", source_pid=attacker_pid, action="attack_me")
        ...
        if target["hp"] <= 0:
            self._queue_ai_alert(target_pid, f"{attacker_pid} knocked me out.", f"{attacker_pid} put me down.", source_pid=attacker_pid, action="knockout_me")
    ...
    def _try_attack_npc(self, attacker_pid, target_owner_pid, target_npc_id):
        ...
        self._queue_ai_alert(target_owner_pid, f"{attacker_pid} hit my NPC {npc_name} for {dmg} damage.", f"Hands off {npc_name}.", source_pid=attacker_pid, action="attack_my_npc")
    ...
    def _npc_attack_player(self, owner_pid, target_pid, npc_str, npc_id):
        ...
        self._queue_ai_alert(target_pid, f"{attacker_name} hit me for {dmg} damage.", f"{attacker_name} is on me.", source_pid=owner_pid, action="attack_me")
    ...
    def _npc_attack_npc(self, owner_pid, attacker_npc_id, target_owner_pid, target_npc_id, npc_str):
        ...
        self._queue_ai_alert(target_owner_pid, f"{attacker_name} hit my NPC {npc_name} for {dmg} damage.", f"{attacker_name} is hitting {npc_name}.", source_pid=owner_pid, action="attack_my_npc")
    ...
    def _apply_blast_mode(cost, dmg, blast_mode):
        ...
        if blast_mode == "scatter_shot":
            return cost * 2, max(1, int(dmg * 0.6))  # 3 projectiles × 60% each ≈ 180% total
    ...
    def _ki_blast_player(self, attacker_pid, target_pid, blast_mode=""):
        ...
        self._queue_ai_alert(target_pid, f"{attacker_pid} hit me with a ki blast for {final_dmg} damage.", f"{attacker_pid} blasted me.", source_pid=attacker_pid, action="attack_me")

        self._grant_xp(attacker, 8)

        if target["hp"] <= 0:
            self._queue_ai_alert(target_pid, f"{attacker_pid} knocked me out with a ki blast.", f"{attacker_pid} blasted me down.", source_pid=attacker_pid, action="knockout_me")
            self._knock_out_player(target)
    ...
    def _ki_blast_npc(self, attacker_pid, target_owner_pid, target_npc_id, blast_mode=""):
        ...
        npc_state["_last_attacked_by"] = {"type": "player", "id": attacker_pid}
        npc_name = npc_state.get("name", target_npc_id)
        self._queue_ai_alert(target_owner_pid, f"{attacker_pid} blasted my NPC {npc_name} for {final_dmg} damage.", f"{attacker_pid} blasted {npc_name}.", source_pid=attacker_pid, action="attack_my_npc")
    ...
    def _ki_blast_dummy(self, pid, dummy_id, blast_mode=""):
        ...
        if dummy.get("owner"):
            self._queue_ai_alert(dummy.get("owner"), f"{pid} hit my training dummy with a ki blast.", f"{pid} is hitting my dummy.", source_pid=pid, action="hit_my_dummy")
        self._grant_xp(p, 3)
        if dummy["hp"] <= 0:
            if dummy.get("owner"):
                self._queue_ai_alert(dummy.get("owner"), f"{pid} destroyed my training dummy.", f"{pid} broke my dummy.", source_pid=pid, action="hit_my_dummy")
            dummy["dead"] = True
    ...
    def _ki_blast_ground_item(self, pid, item_id, blast_mode=""):
        ...
        self.ground_items = [gi for gi in self.ground_items if gi["id"] != item_id]
    ...
    def _npc_ki_blast_player(self, owner_pid, npc_id, target_pid):
        ...
        target["hp"] = max(0, target["hp"] - final_dmg)
        attacker_name = npc_state.get("name", npc_id)
        self._queue_ai_alert(target_pid, f"{attacker_name} hit me with a ki blast for {final_dmg} damage.", f"{attacker_name} blasted me.", source_pid=owner_pid, action="attack_me")
        self._grant_xp(npc_state, 8)

        if target["hp"] <= 0:
            if owner:
                self._grant_xp(owner, PVP_XP_KILL)
            self._queue_ai_alert(target_pid, f"{attacker_name} knocked me out with a ki blast.", f"{attacker_name} blasted me down.", source_pid=owner_pid, action="knockout_me")
            self._knock_out_player(target)
    ...
    def _npc_ki_blast_npc(self, owner_pid, attacker_npc_id, target_owner_pid, target_npc_id):
        ...
        npc_name = npc_state.get("name", target_npc_id)
        attacker_name = attacker_npc.get("name", attacker_npc_id)
        self._queue_ai_alert(target_owner_pid, f"{attacker_name} blasted my NPC {npc_name} for {final_dmg} damage.", f"{attacker_name} blasted {npc_name}.", source_pid=owner_pid, action="attack_my_npc")
        self._grant_xp(attacker_npc, 5)
    ...
    def _apply_player_knockout_penalty(self, target):
        ...
        self._ensure_level_based_ki(target, refill=False)
    ...
    def _try_plant_seed(self, pid, x, y):
        ...
        if result == "planted":
            ...
            self.fx_events.append({"type": "chat_hint", "pid": pid, "text": "Can only plant on fertile soil tiles."})
        elif result == "occupied":
            self.fx_events.append({"type": "chat_hint", "pid": pid, "text": "Something is already growing there."})
    ...
    def _try_harvest_crop(self, pid, crop_id):
        ...
        if result:
            ...
            self.fx_events.append({"type": "crop_harvested", "crop_id": crop_id, "harvester": pid})
    ...
    def _try_build_ki_target(self, pid, x=None, y=None):
        ...
        print(f"[game_state] {pid} built ki target {item_id} at ({x:.0f}, {y:.0f})")
        return item_id
    ...
    def _ki_blast_ki_target(self, pid, target_id, watching_npc_id=None):
        ...
        if broke:
            self.ground_items = [item for item in self.ground_items if item.get("id") != target_id]
    ...
    def _npc_mine_rock(self, pid, npc_id, rock_id):
        ...
        if rock["hits_left"] <= 0:
            rock["mined"] = True
        print(f"[game_state] NPC {npc_id} mined rock {rock_id}, stones={npc_state['stones']}")
    ...
    # ── Building processing (log cutters + minecart tracks) ─────────────────
    ...
    def _tick_buildings(self, dt, now):
        ...
        for bid, b in list(self.buildings.items()):
            kind = b["kind"]
