...
class GameState:
    ...
    # ── Build NPC (delegated to NPCManager) ────────────────────────────────────

    def _try_build_npc(self, pid, name=None):
        return self.npc_manager._try_build_npc(pid, name)

    # ── NPC sync (delegated to NPCManager) ────────────────────────────────────

    def _sync_player_npcs(self, pid, npcs):
        return self.npc_manager._sync_player_npcs(pid, npcs)
    ...
    def _try_give_npc_equipment(self, pid, npc_id, eq_id):
        return self.npc_manager._try_give_npc_equipment(pid, npc_id, eq_id)

    def _try_take_npc_equipment(self, pid, npc_id, slot):
        return self.npc_manager._try_take_npc_equipment(pid, npc_id, slot)
    ...
    # ── Background NPC worker simulation (delegated to NPCManager) ────────────
